...
export const registerOrLoginWithEmail = async (req, res) => {
  try {
    ...
    if (user) {
      ...
      return res.status(200).json({ status: "success", message: "Login successful", user: { email: user.email, name: user.name } });
    }
    ...
    res.status(200).json({ status: "success", message: "Registration successful", user: { email, name } });
  } catch (err) {
    console.error(err);
